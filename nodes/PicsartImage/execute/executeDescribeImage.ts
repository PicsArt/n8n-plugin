...
export async function executeDescribeImage(
	...
	// Prepare form data for image description
	...
	try {
		// Call Picsart API to describe image
		result = await context.helpers.httpRequestWithAuthentication.call(
			context,
			'picsartApi',
			{
				method: 'POST',
				url: 'https://api.picsart.io/tools/1.0/describe',
				headers: {
	...
	// Extract the description text from the result
	const description = result?.data?.description || result?.description || result?.data || 'No description available';
