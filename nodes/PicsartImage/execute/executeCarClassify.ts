...
export async function executeCarClassify(
	...
	if (resource === 'DATA') {
		...
		try {
			...
			fileName = binaryData.fileName || (binaryData.fileExtension ? `image.${binaryData.fileExtension}` : 'car-image.png');
	...
	// Prepare form data for car classification
	...
	try {
		// Call Picsart API to classify car image
		result = await context.helpers.httpRequestWithAuthentication.call(
			context,
			'picsartApi',
			{
				method: 'POST',
				url: 'https://api.picsart.io/tools/1.0/cars/classify',
				headers: {
	...
	// The API returns categories like: exterior, interior, engine, undercarriage, other
	returnData.push({
		json: {
			imageSource: binaryDataBuffer ? `[Binary: ${fileName}]` : imageUrl,
			classification: result?.data || result,
