...
export async function executeResizeImage(
	...
	// Prepare form data for resizing
	...
	try {
		// Call Picsart API to resize image
		...
		returnData.push({
			binary: {
				data: await context.helpers.prepareBinaryData(imageBuffer, `resized-image.${format.toLowerCase()}`),
			},
			json: {
