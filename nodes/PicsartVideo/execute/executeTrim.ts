...
const TRIM_API_URL = 'https://video-api.picsart.io/v1/trim';
...
export async function executeTrim(
	...
	const startMs = context.getNodeParameter('start', itemIndex, 0) as number;
	const mode = context.getNodeParameter('mode', itemIndex, 'simple') as string;
	const endMs = context.getNodeParameter('end', itemIndex, 1) as number;
	...
	if (endMs <= startMs) {
		throw new NodeOperationError(
			context.getNode(),
			'End must be greater than start (both in milliseconds)',
			{ itemIndex },
		);
	}
		...
		if (transactionId && !resultVideoUrl) {
			...
			while (pollAttempts < maxPollAttempts) {
				...
				if (result.status === 'failed' || result.status === 'error') {
					throw new NodeOperationError(
						context.getNode(),
						`Trim failed: ${result.message || result.error || result.data?.message || 'Unknown error'}`,
						{ itemIndex },
					);
				}
				pollAttempts++;
			...
			if (!resultVideoUrl) {
				throw new NodeOperationError(
					context.getNode(),
					`Trim timed out after ${maxPollAttempts} polling attempts. Transaction ID: ${transactionId}.`,
					{ itemIndex },
				);
			}
		}
