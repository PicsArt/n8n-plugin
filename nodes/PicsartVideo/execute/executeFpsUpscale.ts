...
const FPS_UPSCALE_API_URL = 'https://video-api.picsart.io/v1/upscale/fps';
const VIDEO_RESULT_URL = 'https://video-api.picsart.io/v1/video';
const maxPollAttempts = 600;

export async function executeFpsUpscale(
		...
		if (transactionId && !resultVideoUrl) {
			...
			while (pollAttempts < maxPollAttempts) {
				...
				if (result.status === 'failed' || result.status === 'error') {
					throw new NodeOperationError(
						context.getNode(),
						`FPS upscale failed: ${result.message || result.error || result.data?.message || 'Unknown error'}`,
						{ itemIndex },
					);
				}
				pollAttempts++;
			...
			if (!resultVideoUrl) {
				throw new NodeOperationError(
					context.getNode(),
					`FPS upscale timed out after ${maxPollAttempts} polling attempts. Transaction ID: ${transactionId}.`,
					{ itemIndex },
				);
			}
		}
